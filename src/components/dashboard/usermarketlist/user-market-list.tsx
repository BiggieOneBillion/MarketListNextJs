...
const UserMarketList = () => {
  ...
  const record: { i: string; indexs: number[] }[] = [];
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await axios.get(`/api/market-list/get-all-user-list`, {
        headers: {
          ...
          {width < 768 ? (
            <section className="flex items-center gap-2 w-full md:hidden">
              ...
              <div className="md:hidden">
                <MorePopOverMenu
                  id={info.data[index]["_id"] as unknown as number}
                />
              </div>
            </section>
          ) : (
